...
import numpy as np
import pandas as pd
from pygridmappr.core import points_to_grid
...
spacers = [
    ...
    (11, 2),
    (11, 3),
    (11, 4),  # Bottom row, left side
    (12, 1),
    (12, 2),
    (12, 3),
    (12, 4),  # Second row, left side
]
...
mainland_assignments = result_no_spacers[
    result_no_spacers["area_name"].str.startswith("M")
]
print(mainland_assignments[["area_name", "row", "col"]].to_string(index=False))
print("\nIsland assignments:")
island_assignments = result_no_spacers[
    result_no_spacers["area_name"].str.startswith("I")
]
...
result_with_spacers = points_to_grid(
    pts, n_row=12, n_col=10, compactness=0.5, spacers=spacers
)
print("Mainland assignments:")
mainland_assignments = result_with_spacers[
    result_with_spacers["area_name"].str.startswith("M")
]
print(mainland_assignments[["area_name", "row", "col"]].to_string(index=False))
print("\nIsland assignments:")
island_assignments = result_with_spacers[
    result_with_spacers["area_name"].str.startswith("I")
]
print(island_assignments[["area_name", "row", "col"]].to_string(index=False))
print()
...
island_cells_with_spacers = set(
    zip(island_assignments["row"], island_assignments["col"])
)
mainland_cells_with_spacers = set(
    zip(mainland_assignments["row"], mainland_assignments["col"])
)
