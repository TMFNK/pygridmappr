...
def create_grid_layout(
    n_row: int, n_col: int, spacers: Optional[List[Tuple[int, int]]] = None
) -> pd.DataFrame:
    """
...
def compare_compactness(
    ...
    n_row: int,
    n_col: int,
    compactness_values: Optional[List[float]] = None,
    spacers: Optional[List[Tuple[int, int]]] = None,
...
def generate_sample_points(
    n_points: int = 50, pattern: str = "random", seed: Optional[int] = None
) -> pd.DataFrame:
    """
    ...
    return pd.DataFrame(
        {"area_name": [f"P{i + 1}" for i in range(len(x))], "x": x, "y": y}
    )
